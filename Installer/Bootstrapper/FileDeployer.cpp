...
std::wstring FileDeployer::downloadVersionedFile(const TCHAR* name, Progress& progress, bool usePrimaryCdn)
{
	...
	int status_code;
	{
		...
		if(usePrimaryCdn)
		{
			status_code = HttpTools::httpGetCdn(_site, _site->InstallHost(), path, etag, result, false, E:/boost/::bind(&Progress::update, &progress, _site, _1, _2));
		}
		else
		{
			status_code = HttpTools::httpGet(_site, _site->InstallHost(), path, etag, result, false, E:/boost/::bind(&Progress::update, &progress, _site, _1, _2));
		}
	}
