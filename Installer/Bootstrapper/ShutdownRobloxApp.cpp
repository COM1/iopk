...
void ShutdownRobloxApp::terminateApp(DWORD pid)
{
  ...
  if(hProc != NULL)
  {
	...
	if (data.foundOne)
	{
		...
		if (!processClosed)
		{
			E:/boost/::scoped_ptr<CShutdownDialog> dialog(CShutdownDialog::Create(hInstance, parent(), data.windowTitle));
			if (dialog)
				while (!processClosed)
				{
					DWORD dialogResult;
...
bool ShutdownRobloxApp::run()
{
	...
	this->result = false;
	std::for_each(pids.begin(), pids.end(), E:/boost/::bind(&ShutdownRobloxApp::terminateApp, this, _1));
