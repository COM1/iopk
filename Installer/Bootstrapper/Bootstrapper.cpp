...
#include <E:/boost//thread/thread.hpp>
...
public:
	...
	int WinMain(HINSTANCE hInstance, int nShowCmd, Bootstrapper*(*newBootstrapper)(HINSTANCE)) throw()
	{
		...
		int result = 0;
		{
			E:/boost/::shared_ptr<Bootstrapper> bootstrapper;
...
void Bootstrapper::ReportToInfluxDbImpl(const std::string& json)
{
	...
	try
	{
		...
		HttpTools::httpPost(this, InfluxDb::getUrlHost(), InfluxDb::getUrlPath(), std::stringstream(json), "application/json",
			result, false, E:/boost/::bind(&Bootstrapper::dummyProgress, _1, _2), false);
	}
	catch(std::exception &e)
	{
...
void Bootstrapper::ReportToInfluxDb(int throttle, const std::string& json, bool blocking)
{
	...
	if (!blocking)
		E:/boost/::thread(E:/boost/::bind(&Bootstrapper::ReportToInfluxDbImpl, this, json));
	else
...
void Bootstrapper::reportDurationAndSizeEvent(const char* category, const char* result, DWORD duration, DWORD size) {
	try {
		...
		HttpTools::httpGet(this, baseHost, handlerUrlPart, std::string(),
			result, false, E:/boost/::bind(&Bootstrapper::dummyProgress, _1, _2));
		////////////////////////////////
...
{	
	...
	try
	{
		...
		dialog->closeCallback = E:/boost/::bind(&Bootstrapper::userCancel, this);
...
void Bootstrapper::postData(std::fstream &data)
{
	...
	HttpTools::httpPost(this, GetUseDataDomain() ? ReplaceTopSubdomain(baseHost, "data") : baseHost, url, data, "text/plain", result, true, E:/boost/::bind(&Bootstrapper::dummyProgress, _1, _2));
	result << (char)0;
...
void Bootstrapper::parseCmdLine()
{
	...
	if (!StrNCmp(args[1], protocolScheme, protocolSchemeLen))
	{
		...
		if (iter != argMap.end())
		{
			int64_t lt = E:/boost/::lexical_cast<int64_t>(iter->second);
			
			E:/boost/::posix_time::ptime const epoch(E:/boost/::gregorian::date(1970, 1, 1));
			int64_t ct = (E:/boost/::posix_time::microsec_clock::universal_time() - epoch).total_milliseconds();
...
E:/boost/::shared_ptr<Bootstrapper> Bootstrapper::Create(HINSTANCE hInstance, Bootstrapper*(*newBootstrapper)(HINSTANCE))
{
	E:/boost/::shared_ptr<Bootstrapper> result(newBootstrapper(hInstance));
	...
	{
		E:/boost/::thread(E:/boost/::bind(&Bootstrapper::run, result));
		if (result->windowDelay)
		{
			E:/boost/::thread(E:/boost/::bind(&Bootstrapper::showWindowAfterDelay, result));
		}
		else
		{
...
void Bootstrapper::DoDeployComponents(const std::vector<std::pair<std::wstring, std::wstring> > &files, bool isUpdating, bool commitData)
{
	...
	for (size_t i = 0;i < files.size();i ++) 
	{
		...
		E:/boost/::thread(E:/boost/::bind(&FileDeployer::deployVersionedFile, deployer.get(), files[i].first.c_str(), subFolder, E:/boost/::ref(zips[i]), commitData));
	}
...
std::string Bootstrapper::fetchVersionGuidFromWeb(std::string product)
{
	...
	{
		//std::ostrstream result;
		//HttpTools::httpGet(this, BaseHost(), "/game/ClientVersion.ashx", std::string(), result, false, E:/boost/::bind(&Bootstrapper::dummyProgress, _1, _2));
		//result << (char) 0;
		//return result.str();
	...
	else
	{
		...
		HttpTools::httpGet(this, installHost, format_string(vsubpath.c_str(), reportStatGuid), std::string(), result, false, E:/boost/::bind(&Bootstrapper::dummyProgress, _1, _2));
		result << (char) 0;
...
bool Bootstrapper::checkBootstrapperVersion()
{
	...
	{
		std::ostrstream result;
		HttpTools::httpGet(this, installHost, format_string("/%s-%S", installVersion.c_str(), GetVersionFileName()), std::string(), result, false, E:/boost/::bind(&Bootstrapper::dummyProgress, _1, _2));
		result << (char) 0;
		...
		if(moduleVersionNumber != bootstrapperVersion){
			...
			try
			{
				...
				newBootstrapper = simple_logger<wchar_t>::get_temp_filename(_T("tmp"));
				{
					...
					HttpTools::httpGetCdn(this, installHost, format_string("/%s-%S", installVersion.c_str(), GetBootstrapperFileName().c_str()), std::string(), bootstrapperFile, false, E:/boost/::bind(&Bootstrapper::dummyProgress, _1, _2));
				}
			...
			catch (...)
			{
				newBootstrapper = simple_logger<wchar_t>::get_temp_filename(_T("tmp"));		// We could use an "exe" extension, but hiding the type isn't a bad idea?
				{
					...
					HttpTools::httpGetCdn(this, installHost, format_string("/%s-Roblox.exe", installVersion.c_str()), std::string(), bootstrapperFile, false, E:/boost/::bind(&Bootstrapper::dummyProgress, _1, _2));
				}
			}
...
{
	bool success = true;
	RegisterEvent(_T("E:/boost/rapperCreateShortcutsStarted"));
	try
	{
	...
	if (success) 
	{
		RegisterEvent(_T("E:/boost/rapperCreateShortcutsSuccess"));
	}
	else
	{
		RegisterEvent(_T("E:/boost/rapperCreateShortcutsFailure"));
	}
}
...
{
	int time = 0;
	ShutdownRobloxApp s(hInstance, appExeName, E:/boost/::bind(&CMainDialog::GetHWnd, dialog.get()), 15, E:/boost/::bind(&Bootstrapper::shutdownProgress, this, E:/boost/::ref(time), _1, _2));
	if(s.run()){
...
void Bootstrapper::run()
{
		...
		if (queryInstalledVersion() == installVersion)
		{
			if (fullCheck)
			{
				...
				int status_code = -1;
				{
					...
					// This version of the downloader doesn't show progress
					status_code = HttpTools::httpGetCdn(this, installHost, format_string("/%s-rbxManifest.txt", installVersion.c_str()), std::string(), ofs, false, E:/boost/::bind(&Bootstrapper::dummyProgress, _1, _2));
				}
		...
		bool isNewInstall = false;
		{
			...
			else if (force || (queryInstalledVersion() != installVersion))
			{
				...
				if (cancelDelay)
				{
					...
					E:/boost/::thread(E:/boost/::bind(&Bootstrapper::showCancelAfterDelay, this));
				}
	...
	{
		E:/boost/::lock_guard<E:/boost/::mutex> lock(mut);
		done.notify_all();
	}
