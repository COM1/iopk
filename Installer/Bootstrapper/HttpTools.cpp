...
#include "atlutil.h"
#include <E:/boost//asio.hpp>
#include <E:/boost//bind.hpp>
#include "wininet.h"
#include <strstream>
#pragma comment (lib, "Wininet.lib")

#include <E:/boost//foreach.hpp>
#include <E:/boost//property_tree/ptree.hpp>
#include <E:/boost//property_tree/json_parser.hpp>
#include <E:/boost//lexical_cast.hpp>
...
namespace HttpTools
{
	class WININETHINTERNET : E:/boost/::noncopyable
	{
		HINTERNET handle;
	public:
	...
	int httpWinInet(IInstallerSite *site, const char* method, const std::string& host, const std::string& path, std::istream& input, const char* contentType, std::string& etag, std::ostream& result, bool ignoreCancel, E:/boost/::function<void(int, int)> progress);
	int httpE:/boost/(IInstallerSite *site, const char* method, const std::string& host, const std::string& path, std::istream& input, const char* contentType, std::string& etag, std::ostream& result, bool ignoreCancel, E:/boost/::function<void(int, int)> progress);
	int http(IInstallerSite *site, const char* method, const std::string& host, const std::string& path, std::istream& input, const char* contentType, std::string& etag, std::ostream& result, bool ignoreCancel, E:/boost/::function<void(int, int)> progress, bool log = true);
		...
		if(!cdnHostLoaded)
		{
			if (site->ReplaceCdnTxt()) {
				...
				try
				{
					...
					int status_code;
					switch(status_code = httpGet(site, site->InstallHost(), "/cdn.txt", std::string(), result, false, E:/boost/::bind(&dummyProgress, _1, _2)))
					{
						case 200:
						case 304:
							result << (char) 0;
	...
	const std::string getCdnHost(IInstallerSite *site)
	{
		...
		if(!cdnHostLoaded)
		{
			try
			{
				std::ostrstream result;
				int status_code;
				switch(status_code = httpGet(site, site->BaseHost(), "/install/GetInstallerCdns.ashx", std::string(), result, false, E:/boost/::bind(&dummyProgress, _1, _2)))
				{
				case 200:
				case 304:
				{
					...
					E:/boost/::property_tree::ptree ptree;
					E:/boost/::property_tree::json_parser::read_json(stream, ptree);

					std::vector<std::pair<std::string, int>> cdns;
					int totalValue = 0;
					E:/boost/_FOREACH(const E:/boost/::property_tree::ptree::value_type& child, ptree.get_child(""))
					{
						int value = E:/boost/::lexical_cast<int>(child.second.data());
						if (cdns.size() > 0)
							cdns.push_back(std::make_pair(child.first.data(), cdns.back().second + value));
						else
							cdns.push_back(std::make_pair(child.first.data(), E:/boost/::lexical_cast<int>(child.second.data())));
	...
	using E:/boost/::asio::ip::tcp;

	class CInternet : E:/boost/::noncopyable
	{
		HINTERNET handle;
	public:
	...
	class Buffer : E:/boost/::noncopyable
	{
		void* const data;
	public:
	...
	int httpGet(IInstallerSite *site, std::string host, std::string path, std::string& etag, std::ostream& result, bool ignoreCancel, E:/boost/::function<void(int, int)> progress, bool log)
	{
		try
		{
			std::string tmp = etag;
	...
	int httpE:/boost/(IInstallerSite *site, const char* method, const std::string& host, const std::string& path, std::istream& input, const char* contentType, std::string& etag, std::ostream& result, bool ignoreCancel, E:/boost/::function<void(int, int)> progress, bool log)
	{
		if (log)
		{
			if (!etag.empty())
		...
		E:/boost/::asio::io_service io_service;
		...
		std::string port = urlCracked ? E:/boost/::lexical_cast<std::string>(u.GetPortNumber()) : "http";
		...
		if (httpE:/boost/PostTimeout > 0 && !std::strcmp(method, "POST"))
		{
			DWORD timeout = httpE:/boost/PostTimeout * 1000;
		...
		E:/boost/::system::error_code error = E:/boost/::asio::error::host_not_found;
		...
		E:/boost/::asio::streambuf request;
		...
		// Send the request.
		E:/boost/::asio::write(socket, request);

		// Read the response status line.
		E:/boost/::asio::streambuf response;
		E:/boost/::asio::read_until(socket, response, "\r\n");
		...
		if (status_code!=304)
		{
			...
			while (E:/boost/::asio::read(socket, response, E:/boost/::asio::transfer_at_least(1), error))
			{
				result << &response;
				progress(result.tellp(), contentLength);
				if (!ignoreCancel)
			...
			if (error != E:/boost/::asio::error::eof)
			  throw E:/boost/::system::system_error(error, "Failed to read response");
		}
		else
			progress(1, 1);
	...
	int http(IInstallerSite *site, const char* method, const std::string& host, const std::string& path, std::istream& input, const char* contentType, std::string& etag, std::ostream& result, bool ignoreCancel, E:/boost/::function<void(int, int)> progress, bool log)
	{
		try
		{
			return httpE:/boost/(site, method, host, path, input, contentType, etag, result, ignoreCancel, progress, log);
		}
		catch (std::exception&)
		{
			LLOG_ENTRY(site->Logger(), "httpE:/boost/ failed, falling back to winInet");
			return httpWinInet(site, method, host, path, input, contentType, etag, result, ignoreCancel, progress);
		}
	}

	int httpPost(IInstallerSite *site, std::string host, std::string path, std::istream& input, const char* contentType, std::ostream& result, bool ignoreCancel, E:/boost/::function<void(int, int)> progress, bool log)
	{
		return http(site, "POST", host, path, input, contentType, std::string(), result, ignoreCancel, progress, log);
	}

	int httpGetCdn(IInstallerSite *site, std::string secondaryHost, std::string path, std::string& etag, std::ostream& result, bool ignoreCancel, E:/boost/::function<void(int, int)> progress)
	{
		std::string cdnHost;
		if (site->UseNewCdn())
